...
csv_file=open('lung_nodule_annotation_1.0.csv', mode='w+')
...
######################################################################################			
			...
			for h in range(total_dfAcc):
				...
				if (x1==0 and x2==0 and y1 ==0 and y2==0) or box_size<16:
					print ('\tNo annotation or NODULE too small (<16pix) = '+str(all_dirs[dir_it])+'.......')
					...
					if z1_slice==z2_slice:
						...
						hdf5_name=str(str(all_dirs[dir_it])+'_'+str(z1_order))
						hdf5_path = '/media/ivanwilliam/Ivan_HDD_2TB/i3d_hdf5_lung_data_resized_2x_box16/HDF5_file/'+str(hdf5_name)+'.h5'
